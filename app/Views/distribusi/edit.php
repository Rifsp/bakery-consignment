<?= view('layout/header', ['title' => 'Edit Distribusi']) ?>
<?= view('layout/sidebar') ?>
<div class="main-content">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 class="mb-0">Edit Distribusi: <?= $distribusi['kode_distribusi'] ?></h4>
        <a href="distribusi/detail/<?= $distribusi['id'] ?>" class="btn btn-secondary"><i class="bi bi-arrow-left"></i> Kembali</a>
    </div>
    ...
    <div class="card">
        <div class="card-body">
            <form action="distribusi/update/<?= $distribusi['id'] ?>" method="POST">
                <div class="row">
                    <div class="col-md-4 mb-3">
                        <label class="form-label">Sales</label>
                        <select name="id_sales" class="form-control" required>
                            <?php foreach ($sales_list as $s): ?>
                                <option value="<?= $s['id'] ?>" <?= $distribusi['id_sales'] == $s['id'] ? 'selected' : '' ?>><?= $s['nama_sales'] ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="id_warung" class="form-control" required>
                            <?php foreach ($warung_list as $w): ?>
                                <option value="<?= $w['id'] ?>" <?= $distribusi['id_warung'] == $w['id'] ? 'selected' : '' ?>><?= $w['nama_warung'] ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-md-4 mb-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="keterangan" class="form-control" value="<?= $distribusi['keterangan'] ?>">
                </div>
